...
//REV_PUBLIC_FUNCTION(ovrResult) rev_ConnectTouchs()
//{
...
REV_PUBLIC_FUNCTION(bool) rev_EmulateTouchPositionOffset(bool l, float x, float y)
{
	if (g_Session != nullptr)
	{
		if (l)
		{
			g_Session->Input->EmulateTouchsPositionOffset(ovrControllerType_LTouch, x, y);
		}
		else
		{
			g_Session->Input->EmulateTouchsPositionOffset(ovrControllerType_RTouch, x, y);
		}
		return true;
	}
...
//REV_PUBLIC_FUNCTION(ovrResult) rev_DisconnectTouchs()
//{
//	if (!session->Input->DisconnectTouchs())
